...
def main(root):
    ...
    with loc_map:
        ...
        fig = px.scatter_mapbox(data_sel2, 
                                hover_name='매장명',                        
                                ...
                                size = "Wallet Size",
                                zoom=11,
                                lat='위도', lon='경도') # open-street-map, carto_positron
    ...
    if st.checkbox("기존 고객 영업 추천"):
        data_sel = data[data["category"]=="FWPOS"]
        ...
        data_show.columns = ["매장명","주소","메인 메뉴","메뉴 Raw","고객 잔여 Wallet",
                             "현재 판매 금액","판매 내역",
                             "[추천]판매 가능 식자재",
                             "[추천]판매 가능 상품"]
    ...
    data1 = go.Bar(x=time_dts["month"],y=time_dts["buy"], name="CJ 판매금액",
                   marker={"color":"rgba(208,191,228,1)"},width=0.34)
    data2 = go.Bar(x=time_dts["month"],y=time_dts["sales"], name="식자재 비용",
    ...
    fig3.add_trace(go.Scatter(x=time_dt['date'], y=time_dt['sales'],
                              name="식자재 비용",marker={"color":"rgba(120,35,220,0.8)"},
                              mode = 'lines+markers'
                              ))
    fig3.add_trace(go.Bar(x=time_dt['date'], y=time_dt['buy'],name="CJ 판매금액",
                          marker={"color":"rgba(208,191,228,1)"}))
    
    fig3.update_layout(height=600, width=800, title_text="일별 구매/매출 정보",
    ...
    st.subheader(" ")
    st.subheader('구매/매출 트렌드')
    st.text("기존/잠재 고객의 구매, 매출 트렌드")
    res = master_dt["name"]
    ...
    fig2 = make_subplots(rows=2, cols=1, subplot_titles=["외식업체 매출", "CJ 판매금액"],
                         x_title="Day",y_title="KRW",shared_xaxes=True,
                         vertical_spacing=0.1)
    
    fig2.append_trace(
        go.Bar(x=time_dt['date'], y=time_dt['sales'],name="외식업체 매출",
               marker={"color":"rgba(120,35,220,0.8)"}),row=1,col=1
        )
    fig2.append_trace(
        go.Bar(x=time_dt['date'], y=time_dt['buy'],name="CJ 판매금액",
               marker={"color":"rgba(208,191,228,1)"}),row=2,col=1
        )
    fig2.update_layout(height=600, width=800, title_text="일별 구매 트렌드",
