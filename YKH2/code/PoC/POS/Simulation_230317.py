...
def crawling_and_matching(loc, name, recipe):
    crawled = crawling_menus(loc, name)
...
recipe = pd.read_csv('C:/Users/kyoo02/OneDrive - Kearney/바탕 화면/2023/PJT/CJFreshWay/Code2/c_project/YKH/data/통합레시피_유니크.csv',encoding='euc-kr')
...
def btn0command():
    label = Label(root2, text=crawling_and_matching(entry1.get(), entry2.get(), recipe)["메뉴이름"])
    label.pack()
def btncommand():
    label = Label(root2, text=crawling_and_matching(entry1.get(), entry2.get(), recipe)["재료"])
    label.pack()
...
def btn3command():
    label3 = Label(root, text=simWord(entry1.get())[0])
    label3.pack(side = "top",anchor = 'n')
def btn4command():
    label4 = Label(root, text=simWord(entry1.get())[1])
