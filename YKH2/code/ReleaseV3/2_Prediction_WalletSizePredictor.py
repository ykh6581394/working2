...
def rf_modeling(X,y):
    ...
    print("Best Score by Grid Search : {}".format(grid_search.best_score_))
...
def inference(root, dt_name, model, category):
    ...
    X_var1 = list(basic_var | one_hot_var)
    ...
    tmp_df1= tmp_df1.loc[:,X_var1]
    
    prediction = model.predict(tmp_df1[X_var1])
    
  
    print("Complete Inference")
    pred = pd.DataFrame({'식당': tmp_df['매장'] ,'wallet_size':prediction})
    pred.to_csv("./SaveFile/InferResult.csv",index=False,
                encoding="utf-8-sig")
...
if __name__ == "__main__":
    ...
    if mode =="train":
        ...
        if save_option == "save":
            joblib.dump(model, './SaveFile/wallet_prediction_model.pkl')
        ...
        if model_option == "load":
            model = joblib.load('./SaveFile/wallet_prediction_model.pkl')
